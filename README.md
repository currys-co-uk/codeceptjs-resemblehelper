# @currys-group/codeceptjs-resemblehelper

Helper for resemble.js, used for image comparison in tests with Playwright, Webdriver, Puppeteer, Appium, TestCafe!

@currys-group/codeceptjs-resemblehelper is a [CodeceptJS](https://codecept.io/) helper which can be used to compare screenshots and make the tests fail/pass based on the tolerance allowed.

If two screenshot comparisons have difference greater then the tolerance provided, the test will fail.

NPM package: https://www.npmjs.com/package/@currys-group/codeceptjs-resemblehelper

To install the package, just run `npm install @currys-group/codeceptjs-resemblehelper`.
...
```js
{
  helpers: {
    ResembleHelper: {
      require: "@currys-group/codeceptjs-resemblehelper",
